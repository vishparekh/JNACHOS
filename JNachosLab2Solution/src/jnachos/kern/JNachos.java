...
public abstract class JNachos {
	...
	private static ArrayList<Semaphore> semList;
	...
	public static int add(String sp)
	{
		if(!resourceList.contains(sp))
		{
			Semaphore temp = new Semaphore(sp, 0);
			semList.add(temp);
			resourceList.add(sp);
			return semList.size()-1;
		}
		return semList.indexOf(sp);
	}
